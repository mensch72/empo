...
def compute_dependency_levels_general(successors):
...
def compute_dependency_levels_fast(states, level_fct):
            ...
            for agent_index in human_agent_indices:
                ...
                for possible_goal, _ in possible_goal_generator.generate(state, agent_index):
                    if possible_goal.is_achieved(state):
                        ...
                        v_results[state_index][agent_index][possible_goal] = np.sum(p * q)
...
def compute_human_policy_prior(
    ...
    if parallel and len(states) > 1:
        ...
        params = (human_agent_indices, possible_goal_generator, num_agents, num_actions,
        ...
        # Profiling counters
        if PROFILE_PARALLEL:
            prof_states_parallel = 0
            prof_states_sequential = 0
            prof_batches = 0
            prof_fork_time = 0.0
    ...
    return policy_prior
def compute_sequential(states, V_values, system2_policies, transitions,
                     human_agent_indices, possible_goal_generator,
                     num_agents, num_actions, action_powers,
                     believed_others_policy, beta, gamma):
