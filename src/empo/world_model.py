...
TransitionData = Tuple[
    ActionProfile, List[float], List[State]
]  # (action_profile, probs, successor_states)
...
_worker_n_actions_per_agent: Optional[List[int]] = None
...
def _init_dag_worker(env_pickle: bytes) -> None:
    ...
    """
    global _worker_env, _worker_num_agents, _worker_n_actions_per_agent
    ...
    _worker_n_actions_per_agent = _worker_env.n_actions_per_agent  # type: ignore[union-attr]


def _process_state_actions(
    state: State,
) -> Tuple[State, Set[State], List[TransitionData]]:
    """
    ...
    assert _worker_n_actions_per_agent is not None

    env = _worker_env
    num_agents = _worker_num_agents
    na = _worker_n_actions_per_agent
    total_action_combinations = 1
    for c in na:
        total_action_combinations *= c

    successor_states: Set[State] = set()
    action_data: List[TransitionData] = []

    for combo_idx in range(total_action_combinations):
        # Convert combo_idx to action profile (mixed-radix decomposition)
        action_profile: List[int] = []
        temp = combo_idx
        for i in range(num_agents):
            action_profile.append(temp % na[i])
            temp //= na[i]
...
class WorldModel(gym.Env):
    ...
    def n_actions_per_agent(self) -> List[int]:
        """Return a list of action-space sizes, one per agent.

        The default implementation returns ``[action_space.n] * len(agents)``
        (uniform action space).  Subclasses with agent-dependent action
        spaces should override this property.
        """
        n: int = self.action_space.n  # type: ignore[attr-defined]
        return [n] * len(self.agents)  # type: ignore[attr-defined]
    ...
    def transition_probabilities(
        ...
        raise NotImplementedError(
            "Subclasses must implement transition_probabilities()"
        )
    ...
    def perceived_state(self, state: Any, agent_index: int) -> Any:
        ...
        Note:
            Currently, callers (e.g., heuristic human policy priors) are
            responsible for calling ``perceived_state()`` before evaluating
            the policy.  Future work may integrate this into the shared
            planning interfaces (e.g., ``HumanPolicyPrior`` evaluation in
            Phase 1/Phase 2) so partial observability is applied
            automatically across the framework.
    ...
    def get_dag(
        ...
        Dict[State, int],
        List[List[int]],
        List[List[Tuple[ActionProfile, List[float], List[int]]]],
    ]: ...

    def get_dag(self, return_probabilities: bool = False, quiet: bool = False) -> Union[
        Tuple[List[State], Dict[State, int], List[List[int]]],
        Tuple[
            List[State],
            Dict[State, int],
        ...
        edges: Dict[int, Set[int]] = (
            {}
        )  # edges[i] = set of successor indices (temporary)

        # Store transition probabilities if requested
        temp_transitions: Optional[
            Dict[int, List[Tuple[ActionProfile, List[float], List[State]]]]
        ...
        na: List[int] = self.n_actions_per_agent
        ...
        while queue:
            ...
            try:
                current_timestep = (
                    current_state[0]
                    if isinstance(current_state, (tuple, list))
            ...
            for combo_idx in range(total_combinations):
                # Convert combo_idx to action profile tuple (mixed-radix)
                action_profile: List[int] = []
                temp = combo_idx
                for i in range(num_agents):
                    action_profile.append(temp % na[i])
                ...
                trans_result = self.transition_probabilities(
                    current_state, action_profile
                )
                ...
                if return_probabilities and temp_transitions is not None:
                    ...
                    for prob, successor_state in trans_result:
                        probs.append(prob)
                        succ_states.append(successor_state)
                    temp_transitions[current_idx].append(
                        (action_profile_tuple, probs, succ_states)
                    )
        ...
        while topo_heap:
            ...
            for successor_idx in edges[current_idx]:
                ...
                if in_degree[successor_idx] == 0:
                    heapq.heappush(
                        topo_heap, (discovered_states[successor_idx], successor_idx)
                    )
        ...
        transitions: List[List[Tuple[ActionProfile, List[float], List[int]]]] = [
            [] for _ in range(num_states)
        ]
        for old_idx in range(num_states):
            new_idx = old_to_new[old_idx]
            for action_prof, trans_probs, trans_succ_states in temp_transitions[
                old_idx
            ]:
    ...
    def get_dag_parallel(
        ...
        temp_transitions: Dict[State, List[TransitionData]] = (
            {}
        )  # state -> list of (action_profile, probs, succ_states)
        ...
        with ProcessPoolExecutor(
            ...
            while current_wave:
                ...
                futures = {
                    executor.submit(_process_state_actions, state): state
        ...
        num_states = len(discovered_states)
        temp_state_to_idx: Dict[State, int] = {
            state: idx for idx, state in enumerate(discovered_states)
        }
