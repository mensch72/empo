...
class WorldModel(gym.Env):
    ...
    def get_dag(
        ...
        while queue:
            ...
            # Extract remaining steps from state (state[0] for most environments like MultiGrid)
            # Handle states that might not have remaining steps as first element
            try:
                remaining_steps = current_state[0] if isinstance(current_state, (tuple, list)) and len(current_state) > 0 else None
            ...
            if pbar is not None:
                ...
                if remaining_steps is not None:
                    pbar.set_description(f"Building DAG (rem_steps={remaining_steps})")
                pbar.refresh()
