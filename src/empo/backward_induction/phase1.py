...
def _hpp_process_single_state(
    ...
    Args:
        ...
        n_actions_per_agent: Number of actions per agent (list of length num_agents)
        action_powers: Precomputed powers for action profile indexing (mixed-radix)
    ...
    for agent_index in human_agent_indices:
        agent_na = n_actions_per_agent[agent_index]
...
def _hpp_compute_sequential(
    ...
    if use_indexed:
        ...
        pres0 = np.zeros((n_goals, max(n_actions_per_agent)), dtype=np.float16)
        ...
        for timestep in range(disk_dag.max_timestep, -1, -1):
            ...
            for global_state_idx in dag_slice.state_indices:
                ...
                v_results, p_results = _hpp_process_single_state(
                    ...
                    n_actions_per_agent, action_powers, believed_others_policy,
...
def _hpp_process_state_batch(
    ...
    (human_agent_indices, possible_goal_generator, num_agents, n_actions_per_agent, 
    ...
    if _shared_believed_others_policy_pickle is not None:
        ...
        believed_others_policy = DefaultBelievedOthersPolicy(
            num_agents, n_actions_per_agent, human_agent_indices, robot_agent_indices)
...
def compute_human_policy_prior(
    ...
    *,
    ...
    robot_action_profiles: List[List[int]] = [
        list(profile) for profile in product(*[range(n_actions_per_agent[r]) for r in robot_agent_indices])
    ] if robot_agent_indices else [[]]
