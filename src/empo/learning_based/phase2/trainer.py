...
class _MCTSNode:
    ...
    def estimated_q_values(self) -> np.ndarray:
        ...
        if np.any(visited):
            q_values[visited] = (
                self.edge_value_sums[visited] / self.edge_visit_counts[visited]
            )
        return q_values
...
class BasePhase2Trainer(ABC):
    ...
    def __init__(
        ...
        if tensorboard_dir is not None and HAS_TENSORBOARD:
            ...
            if verbose:
                ...
                print(
                    f"[TensorBoard] Created SummaryWriter for pid={os.getpid()} in {tensorboard_dir}"
                )
            ...
            self.writer.add_text(
                "GradNorm/remark", "Gradients are clipped.", global_step=0
            )
    ...
    def _archive_tensorboard_data(self, tensorboard_dir: str) -> None:
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"[TensorBoard] Archived {len(event_files)} old event files to {archive_name}"
                )
            ...
            try:
                ...
                print(
                    f"[TensorBoard] Warning: Could not recreate tensorboard dir: {e2}"
                )
    ...
    def _init_optimizers(self) -> Dict[str, optim.Optimizer]:
        ...
        def get_lr(network, base_lr):
            ...
            if self.config.lookup_use_adaptive_lr and hasattr(
                network, "scale_gradients_by_update_count"
            ):
        ...
        def make_optimizer(network, lr, weight_decay):
            ...
            if len(params) == 0:
                ...
                return optim.Adam(
                    [placeholder], lr=effective_lr, weight_decay=weight_decay
                )
    ...
    def _add_new_lookup_params_to_optimizers(self):
        ...
        if hasattr(self.networks.v_h_e, "get_new_params"):
            new_params_by_optimizer["v_h_e"].extend(
                self.networks.v_h_e.get_new_params()
            )
        if (
            self.config.x_h_use_network
            and self.networks.x_h is not None
            and hasattr(self.networks.x_h, "get_new_params")
        ):
        ...
        if self.debug and total_new > 0:
            ...
            print(
                f"[DEBUG] Added {total_new} new params to optimizers "
    ...
    def _apply_adaptive_lr_scaling(self) -> None:
        ...
        if (
            self.config.x_h_use_network
            and self.networks.x_h is not None
            and hasattr(self.networks.x_h, "scale_gradients_by_update_count")
        ):
            networks_to_scale.append(("x_h", self.networks.x_h))
        if self.config.u_r_use_network and hasattr(
            self.networks.u_r, "scale_gradients_by_update_count"
        ):
            networks_to_scale.append(("u_r", self.networks.u_r))
        if self.config.v_r_use_network and hasattr(
            self.networks.v_r, "scale_gradients_by_update_count"
        ):
    ...
    def _apply_rnd_adaptive_lr_scaling(
        self, states: List[Any], active_networks: set
    ) -> Dict[str, float]:
        """
        ...
        with torch.no_grad():
            ...
            lr_scale = lr_scale.clamp(
                min=self.config.rnd_adaptive_lr_min, max=self.config.rnd_adaptive_lr_max
            )
            ...
            num_clamped_high = (
                (lr_scale == self.config.rnd_adaptive_lr_max).sum().item()
            )
        ...
        detailed_stats = {
            ...
            "frac_clamped_high": (
                num_clamped_high / batch_size if batch_size > 0 else 0.0
            ),
            "running_mean": (
                running_mean.item() if hasattr(running_mean, "item") else running_mean
            ),
        }
    ...
    def step_environment(
        self, state: Any, robot_action: Tuple[int, ...], human_actions: List[int]
    ) -> Any:
        """
    ...
    def _sample_robot_action_with_stats(
        ...
        if torch.rand(1).item() < epsilon:
            return (
                self._sample_robot_exploration_action(
                    state, transition_probs_by_action
                ),
                None,
            )
        ...
        with torch.no_grad():
            q_values = self.networks.q_r_target.forward(state, None, self.device)

            # Add curiosity bonus to Q-values if curiosity is enabled AND transition_probs provided
            if (
                self._curiosity_enabled_for_robot()
                and transition_probs_by_action is not None
            ):
    ...
    def _curiosity_enabled_for_robot(self) -> bool:
        ...
        if (
            self.config.use_count_based_curiosity
            and self.networks.count_curiosity is not None
        ):
    ...
    def _rnd_enabled(self) -> bool:
        ...
        human_rnd = (
            self.config.use_human_action_rnd and self.networks.human_rnd is not None
        )
    ...
    def _sample_robot_exploration_action(
        ...
        if (
            ...
            novelty_scores = self._compute_action_novelty_scores(
    ...
    def _sample_human_actions_for_search(
        ...
        for human_idx in self.human_agent_indices:
            ...
            probs = np.asarray(
                self.human_policy_prior(state, human_idx, goal), dtype=np.float64
            )
            if probs.sum() <= 0:
    ...
    def _evaluate_mcts_node(
        ...
        with torch.no_grad():
            q_values = self.networks.q_r_target.forward(
                node.state, self.env, self.device
            )
            policy = self.networks.q_r_target.get_policy(
                q_values, beta_r=effective_beta_r
            )
            ...
            if self.config.v_r_use_network and self.networks.v_r_target is not None:
                base_state_value = float(
                    self.networks.v_r_target.forward(node.state, self.env, self.device)
                    .reshape(-1)[0]
                    .item()
                )
            else:
        ...
        node.base_q_values = (
            q_values.detach().cpu().numpy().reshape(-1).astype(np.float64)
        )
    ...
    def _run_mcts_policy_search(
        ...
        *,
        ...
        return Phase2SearchStats(
            ...
            root_value=float(
                root.estimated_state_value(temperature=self.config.mcts_temperature)
            ),
    ...
    def _compute_action_novelty_scores(
        self, transition_probs_by_action: Dict[int, List[Tuple[float, Any]]]
    ) -> Optional[np.ndarray]:
        """
    ...
    def sample_human_actions(self, state: Any, goals: Dict[int, Any]) -> List[int]:
        """
        ...
        if use_human_curiosity and len(self.human_agent_indices) > 0:
            try:
                ...
                action_novelties = (
                    self.networks.human_rnd.compute_action_novelties_no_grad(
    ...
    def collect_transition(
        ...
        if self.debug:
            print(
                f"[DEBUG] collect_transition: human_actions={human_actions}, computing transition probs..."
            )
        ...
        if self.config.use_model_based_targets and hasattr(
            self.env, "transition_probabilities"
        ):
        ...
        if self.debug:
            print(
                "[DEBUG] collect_transition: sampling robot action (with transition_probs)..."
            )
        ...
        if self.debug:
            print(
                f"[DEBUG] collect_transition: robot_action={robot_action}, stepping environment..."
            )
        ...
        if self.debug:
            print(
                "[DEBUG] collect_transition: environment stepped, creating transition..."
            )
        ...
        stored_next_state = (
            next_state if self.config.should_store_sampled_next_state() else None
        )
        ...
        transition = Phase2Transition(
            ...
            search_action_value=(
                None if search_stats is None else search_stats.action_values
            ),
        )
    ...
    def _compute_model_based_v_h_e_targets(
        ...
        with torch.no_grad():
            ...
            robot_policies = self.networks.q_r_target.get_policy(
        ...
        if all_next_states:
            with torch.no_grad():
                v_h_e_all = self.networks.v_h_e_target.forward_batch(
                    all_next_states, all_goals, all_human_indices, self.env, self.device
                )
                v_h_e_all = self.networks.v_h_e_target.apply_hard_clamp(
                    v_h_e_all
                ).squeeze()
        ...
        targets = torch.tensor(
            achieved_contributions, device=self.device, dtype=torch.float32
        )
        ...
        if all_next_states:
            ...
            weights = torch.tensor(
                [m[1] for m in successor_mapping],
                device=self.device,
                dtype=torch.float32,
            )
    ...
    def _compute_trajectory_v_h_e_targets(
        ...
        if frontier_states:
            with torch.no_grad():
                ...
                frontier_values = self.networks.v_h_e_target.apply_hard_clamp(
                    frontier_values
                )
            ...
            entry_indices = torch.tensor(
                frontier_entries, device=self.device, dtype=torch.long
            )
            discounts = torch.tensor(
                frontier_discounts, device=self.device, dtype=torch.float32
            )
    ...
    def _compute_trajectory_q_r_targets(
        ...
        if reward_states:
            ...
            entry_indices = torch.tensor(
                reward_entries, device=self.device, dtype=torch.long
            )
            discounts = torch.tensor(
                reward_discounts, device=self.device, dtype=torch.float32
            )
            targets.scatter_add_(0, entry_indices, discounts * u_r_values)

        if frontier_states:
            effective_beta_r = self.config.get_effective_beta_r(
            ...
            with torch.no_grad():
                if self.config.v_r_use_network:
                    ...
                    frontier_u_r = self._compute_u_r_batch_target(
                        frontier_states
                    ).squeeze()
    ...
    def compute_losses(
        ...
        if self.config.u_r_use_network and u_r_active:
            for t in batch:
                if self.config.u_r_sample_humans is None:
                    ...
                    n_sample = min(
                        self.config.u_r_sample_humans, len(self.human_agent_indices)
                    )
                    humans_for_u_r = random.sample(
                        list(self.human_agent_indices), n_sample
                    )
        ...
        if v_h_e_states:
            ...
            with self.profiler.section("target_computation"):
                with torch.no_grad():
                    if self.config.v_h_e_target_mode == "one_step":
                        target_v_h_e = self._compute_model_based_v_h_e_targets(
                            batch, v_h_e_data
                        )
                        v_h_e_target_metrics = {}
                    else:
                        target_v_h_e, v_h_e_target_metrics = (
                            self._compute_trajectory_v_h_e_targets(
                                batch,
                                v_h_e_data,
                                return_metrics=True,
                            )
            ...
            if self.debug and self.training_step_count % 100 == 0:
                ...
                print(
                    f"[DEBUG V_h^e] step={self.training_step_count}, n_samples={n_total}, "
                ...
                for goal_key, data in sorted(goal_targets.items()):
                    ...
                    print(
                        f"[DEBUG V_h^e]   Goal {goal_key}: n={n}, "
            ...
            with torch.no_grad():
                prediction_stats["v_h_e"] = {
                    ...
                    "target_std": (
                        target_v_h_e.std().item() if target_v_h_e.numel() > 1 else 0.0
                    ),
                    **v_h_e_target_metrics,
                }
        ...
        if x_h_active and x_h_states:
            ...
            with self.profiler.section("forward_x_h"):
                x_h_pred = self.networks.x_h.forward_batch(
                    x_h_states, x_h_human_indices, self.env, self.device
                )

            # Target from V_h^e target network
            with torch.no_grad():
                v_h_e_for_x = self.networks.v_h_e_target.forward_batch(
                    x_h_states, x_h_goals, x_h_human_indices, self.env, self.device
                )
            ...
            x_h_weights_tensor = torch.tensor(
                x_h_goal_weights, device=self.device, dtype=torch.float32
            )
            target_x_h = self.networks.x_h.compute_target(
                v_h_e_for_x.squeeze(), x_h_weights_tensor
            )
        ...
        if self.config.u_r_use_network and u_r_active and u_r_flat_states:
            ...
            with torch.no_grad():
                ...
                humans_per_state_t = torch.tensor(
                    u_r_humans_per_state, device=self.device, dtype=torch.float32
                )
        ...
        if q_r_active:
            ...
            if self.config.q_r_target_mode == "one_step":
                ...
                target_q_r_taken = target_q_r_all.gather(
                ...
                with torch.no_grad():
                    target_q_r_taken, q_r_target_metrics = (
                        self._compute_trajectory_q_r_targets(
                            batch,
                            return_metrics=True,
                        )
                    )
            ...
            with torch.no_grad():
                ...
                stats = {
                    ...
                    "std": (
                        q_r_pred_taken.std().item()
                        if q_r_pred_taken.numel() > 1
                        else 0.0
                    ),
                    "target_mean": target_q_r_taken.mean().item(),
                    "target_std": (
                        target_q_r_taken.std().item()
                ...
                # Add z-space statistics when enabled
                if self.config.use_z_space_transform:
                    z_pred_taken = to_z_space(
                        q_r_pred_taken, self.config.eta, self.config.xi
                    )
                    z_target_taken = to_z_space(
                        target_q_r_taken, self.config.eta, self.config.xi
                    )
                    stats["z_mean"] = z_pred_taken.mean().item()
                    stats["z_target_mean"] = z_target_taken.mean().item()
                    stats["in_decay_phase"] = (
                        1.0
                        if self.config.is_in_decay_phase(self.training_step_count)
                        else 0.0
                    )
        ...
        if self.config.v_r_use_network and v_r_active:
            with self.profiler.section("forward_v_r"):
                v_r_pred = self.networks.v_r.forward_batch(
                    states, self.env, self.device
                )
            ...
            with torch.no_grad():
                ...
                pi_r = self.networks.q_r_target.get_policy(
                    q_r_for_v, beta_r=effective_beta_r
                )
            ...
            with torch.no_grad():
                ...
                # Add z-space statistics when enabled
                if self.config.use_z_space_transform:
                    z_pred_stat = to_z_space(
                        v_r_pred.squeeze(), self.config.eta, self.config.xi
                    )
                    z_target_stat = to_z_space(
                        target_v_r, self.config.eta, self.config.xi
                    )
                    stats["z_mean"] = z_pred_stat.mean().item()
                    stats["z_target_mean"] = z_target_stat.mean().item()
        ...
        if (
            ...
            with torch.no_grad():
                ...
                prediction_stats["human_rnd"] = {
                    ...
                    "human_rnd_batch_raw_mean": human_rnd_stats[
                        "human_rnd_batch_raw_mean"
                    ],
                    "human_rnd_batch_raw_std": human_rnd_stats[
                        "human_rnd_batch_raw_std"
                    ],
                }
    ...
    def _compute_u_r_batch_target(self, states: List[Any]) -> torch.Tensor:
        ...
        """
        if self.config.u_r_use_network:
            _, u_r = self.networks.u_r_target.forward_batch(
                states, self.env, self.device
            )
            return u_r
    ...
    def _compute_human_rnd_loss_batch(
        self, batch: List["Phase2Transition"]
    ) -> torch.Tensor:
        """
        ...
        for transition in batch:
            ...
            for idx, (h, action) in enumerate(
                zip(self.human_agent_indices, human_actions)
            ):
    ...
    def get_state_features_for_rnd(
        self, states: List[Any], encoder_coefficients: Optional[List[float]] = None
    ...
    def compute_novelty_for_states(self, states: List[Any]) -> torch.Tensor:
        ...
        if (
            ...
            if not self.config.include_step_count:
                states_for_counts = [
                    s[1:] if isinstance(s, tuple) and len(s) >= 2 else s for s in states
                ]
            else:
    ...
    def record_state_visit(self, state: Any) -> None:
        ...
        try:
            ...
            if (
                not self.config.include_step_count
            ...
            if isinstance(state, tuple) and len(state) >= 2:
                ...
                if isinstance(agent_states, (tuple, list)) and len(agent_states) > 0:
                    ...
                    self._position_visit_counts[position_key] = (
                        self._position_visit_counts.get(position_key, 0) + 1
                    )
        except TypeError:
    ...
    def get_position_visit_summary(self) -> Dict[str, Any]:
        ...
        sorted_counts = sorted(
            self._position_visit_counts.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def training_step(
        ...
        for name, loss in losses.items():
            ...
            if (
                name in self.optimizers
                and loss.requires_grad
                and name in active_networks
            ):
    ...
    class _ActorState:
        ...
        def __init__(
            ...
            self.rollout_step_index = (
                rollout_step_index  # Steps within current replay-linked rollout segment
            )
    ...
    def _actor_step(self, actor_state: "_ActorState") -> Optional[Phase2Transition]:
        ...
        if self.debug and is_terminal:
            print(
                f"[DEBUG] Terminal transition! env_step={actor_state.env_step_count}, "
        ...
        if transition is None:
            ...
            actor_state.goals, actor_state.goal_weights = self._sample_goals(
                actor_state.state
            )
    ...
    def _learner_step(
        ...
        with self.profiler.section("progress_bar"):
            if pbar is not None:
                ...
                x_h_change = abs(
                    current_param_norms.get("x_h", 0)
                    - learner_state.prev_param_norms.get("x_h", 0)
                )
                q_r_change = abs(
                    current_param_norms.get("q_r", 0)
        ...
        with self.profiler.section("tensorboard_logging"):
            if self.writer is not None:
                ...
                for key, value in losses.items():
                    if key == "u_r" and not self.config.u_r_use_network:
                        continue
                    self.writer.add_scalar(
                        f"Loss/{key}", value, self.training_step_count
                    )
                for key, value in grad_norms.items():
                    if key == "u_r" and not self.config.u_r_use_network:
                        continue
                    self.writer.add_scalar(
                        f"GradNorm/{key}", value, self.training_step_count
                    )
                for key, stats in pred_stats.items():
                    ...
                    if key == "rnd":
                        ...
                        if "batch_raw_mean" in stats:
                            self.writer.add_scalar(
                                "Exploration/rnd_raw_novelty_mean",
                                stats["batch_raw_mean"],
                                self.training_step_count,
                            )
                        if "batch_raw_std" in stats:
                            self.writer.add_scalar(
                                "Exploration/rnd_raw_novelty_std",
                                stats["batch_raw_std"],
                                self.training_step_count,
                            )
                        ...
                        if "running_mean" in stats:
                            self.writer.add_scalar(
                                "Exploration/rnd_norm_running_mean",
                                stats["running_mean"],
                                self.training_step_count,
                            )
                        if "running_std" in stats:
                            self.writer.add_scalar(
                                "Exploration/rnd_norm_running_std",
                                stats["running_std"],
                                self.training_step_count,
                            )
                        continue
                    ...
                    if key == "count_curiosity":
                        if "count_curiosity_unique_states" in stats:
                            self.writer.add_scalar(
                                "Exploration/count_unique_states",
                                stats["count_curiosity_unique_states"],
                                self.training_step_count,
                            )
                        if "count_curiosity_total_visits" in stats:
                            self.writer.add_scalar(
                                "Exploration/count_total_visits",
                                stats["count_curiosity_total_visits"],
                                self.training_step_count,
                            )
                        if "count_curiosity_mean_visits" in stats:
                            self.writer.add_scalar(
                                "Exploration/count_mean_visits",
                                stats["count_curiosity_mean_visits"],
                                self.training_step_count,
                            )
                        if "count_curiosity_max_visits" in stats:
                            self.writer.add_scalar(
                                "Exploration/count_max_visits",
                                stats["count_curiosity_max_visits"],
                                self.training_step_count,
                            )
                        if "count_curiosity_min_visits" in stats:
                            self.writer.add_scalar(
                                "Exploration/count_min_visits",
                                stats["count_curiosity_min_visits"],
                                self.training_step_count,
                            )
                        continue
                    ...
                    if key == "human_rnd":
                        if "human_rnd_batch_raw_mean" in stats:
                            self.writer.add_scalar(
                                "Exploration/human_rnd_raw_novelty_mean",
                                stats["human_rnd_batch_raw_mean"],
                                self.training_step_count,
                            )
                        if "human_rnd_batch_raw_std" in stats:
                            self.writer.add_scalar(
                                "Exploration/human_rnd_raw_novelty_std",
                                stats["human_rnd_batch_raw_std"],
                                self.training_step_count,
                            )
                        if "human_rnd_running_mean" in stats:
                            self.writer.add_scalar(
                                "Exploration/human_rnd_norm_running_mean",
                                stats["human_rnd_running_mean"],
                                self.training_step_count,
                            )
                        if "human_rnd_running_std" in stats:
                            self.writer.add_scalar(
                                "Exploration/human_rnd_norm_running_std",
                                stats["human_rnd_running_std"],
                                self.training_step_count,
                            )
                        continue
                    ...
                    if key == "rnd_adaptive_lr":
                        for net_name, net_stats in stats.items():
                            ...
                            if "scale_mean" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_scale_mean",
                                    net_stats["scale_mean"],
                                    self.training_step_count,
                                )
                            # Unclamped stats show the raw variance in uncertainty estimates
                            if "scale_unclamped_std" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_scale_std",
                                    net_stats["scale_unclamped_std"],
                                    self.training_step_count,
                                )
                            if "scale_unclamped_min" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_scale_min",
                                    net_stats["scale_unclamped_min"],
                                    self.training_step_count,
                                )
                            if "scale_unclamped_max" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_scale_max",
                                    net_stats["scale_unclamped_max"],
                                    self.training_step_count,
                                )
                            # Raw RND MSE values (most important for diagnosing RND behavior)
                            if "rnd_mse_mean" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_mse_mean",
                                    net_stats["rnd_mse_mean"],
                                    self.training_step_count,
                                )
                            if "rnd_mse_std" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_mse_std",
                                    net_stats["rnd_mse_std"],
                                    self.training_step_count,
                                )
                            if "rnd_mse_min" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_mse_min",
                                    net_stats["rnd_mse_min"],
                                    self.training_step_count,
                                )
                            if "rnd_mse_max" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_mse_max",
                                    net_stats["rnd_mse_max"],
                                    self.training_step_count,
                                )
                            # Clamping fraction shows if we're hitting limits
                            if "frac_clamped_low" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_frac_clamped_low",
                                    net_stats["frac_clamped_low"],
                                    self.training_step_count,
                                )
                            if "frac_clamped_high" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_frac_clamped_high",
                                    net_stats["frac_clamped_high"],
                                    self.training_step_count,
                                )
                            # Running mean for context on normalization
                            if "running_mean" in net_stats:
                                self.writer.add_scalar(
                                    f"AdaptiveLR/rnd_{net_name}_running_mean",
                                    net_stats["running_mean"],
                                    self.training_step_count,
                                )
                        continue
                    ...
                    if key == "u_r" and "y_mean" in stats:
                        self.writer.add_scalar(
                            "Predictions/u_r_y_mean",
                            stats["y_mean"],
                            self.training_step_count,
                        )
                        if "y_std" in stats:
                            self.writer.add_scalar(
                                "Predictions/u_r_y_std",
                                stats["y_std"],
                                self.training_step_count,
                            )
                        ...
                        if "u_r_target_mean" in stats:
                            self.writer.add_scalar(
                                "Targets/u_r_mean",
                                stats["u_r_target_mean"],
                                self.training_step_count,
                            )
                    elif "mean" in stats:
                        self.writer.add_scalar(
                            f"Predictions/{key}_mean",
                            stats["mean"],
                            self.training_step_count,
                        )
                        if "std" in stats:
                            self.writer.add_scalar(
                                f"Predictions/{key}_std",
                                stats["std"],
                                self.training_step_count,
                            )
                    ...
                    if "z_mean" in stats:
                        self.writer.add_scalar(
                            f"ZSpace/{key}_z_pred",
                            stats["z_mean"],
                            self.training_step_count,
                        )
                    if "z_target_mean" in stats:
                        self.writer.add_scalar(
                            f"ZSpace/{key}_z_target",
                            stats["z_target_mean"],
                            self.training_step_count,
                        )
                    if "in_decay_phase" in stats:
                        self.writer.add_scalar(
                            "ZSpace/in_decay_phase",
                            stats["in_decay_phase"],
                            self.training_step_count,
                        )
            ...
            for key, value in param_norms.items():
                self.writer.add_scalar(
                    f"ParamNorm/{key}", value, self.training_step_count
                )

            # Log exploration epsilons side by side
            self.writer.add_scalar(
                "Exploration/epsilon_r",
                self.config.get_epsilon_r(self.training_step_count),
                self.training_step_count,
            )
            self.writer.add_scalar(
                "Exploration/epsilon_h",
                self.config.get_epsilon_h(self.training_step_count),
                self.training_step_count,
            )

            # Log unique states seen (works for all modes - neural, tabular, etc.)
            self.writer.add_scalar(
                "Exploration/unique_states_seen",
                len(self._state_visit_counts),
                self.training_step_count,
            )
            ...
            if self._state_visit_counts:
                visit_counts = np.array(
                    list(self._state_visit_counts.values()), dtype=np.float32
                )
                self.writer.add_histogram(
                    "Exploration/visit_count_distribution", visit_counts, global_step=0
                )
            ...
            for net_name in networks_to_log_lr:
                lr = self.config.get_learning_rate(
                    net_name,
                    self.training_step_count,
                    self.update_counts.get(net_name, 0),
                )
                self.writer.add_scalar(
                    f"LearningRate/{net_name}", lr, self.training_step_count
                )

            # Log warm-up phase information
            self.writer.add_scalar(
                "Warmup/effective_beta_r",
                self.config.get_effective_beta_r(self.training_step_count),
                self.training_step_count,
            )
            self.writer.add_scalar(
                "Warmup/is_warmup",
                1.0 if self.config.is_in_warmup(self.training_step_count) else 0.0,
                self.training_step_count,
            )
            active = self.config.get_active_networks(self.training_step_count)
            active_mask = sum(
                2**i
                for i, n in enumerate(["v_h_e", "x_h", "u_r", "q_r", "v_r"])
                if n in active
            )
            self.writer.add_scalar(
                "Warmup/active_networks_mask", active_mask, self.training_step_count
            )
            self.writer.add_scalar(
                "Warmup/stage",
                self.config.get_warmup_stage(self.training_step_count),
                self.training_step_count,
            )
            ...
            if hasattr(self, "get_cache_stats"):
                ...
                for encoder_name, (hits, misses) in cache_stats.items():
                    ...
                    if total > 0:
                        hit_rate = hits / total
                        self.writer.add_scalar(
                            f"Cache/{encoder_name}_hit_rate",
                            hit_rate,
                            self.training_step_count,
                        )
                        self.writer.add_scalar(
                            f"Cache/{encoder_name}_calls",
                            total,
                            self.training_step_count,
                        )
            ...
            if lookup_tables:
                ...
                for name, net in lookup_tables.items():
                    ...
                    total_entries += size
                    self.writer.add_scalar(
                        f"LookupTable/{name}_size", size, self.training_step_count
                    )
                self.writer.add_scalar(
        ...
        with self.profiler.section("warmup_check"):
            ...
            if current_stage != learner_state.prev_stage:
                current_stage_name = self.config.get_warmup_stage_name(
                    self.training_step_count
                )
                ...
                if self.verbose:
                    ...
                    print(
                        f"  {learner_state.prev_stage_name} -> {current_stage_name} ({current_stage_duration:,} steps)"
                    )
                    print(f"  Active networks: {active}")
                    effective_beta = self.config.get_effective_beta_r(
                        self.training_step_count
                    )
                ...
                if self.writer is not None:
                    self.writer.add_scalar(
                        "Warmup/stage_transition", 1.0, self.training_step_count
                    )
                    self.writer.add_text(
                        "Warmup/transitions",
        ...
        with self.profiler.section("lr_decay_check"):
            ...
            if currently_in_decay and not learner_state.in_lr_decay_phase:
                ...
                if self.verbose:
                    ...
                    print(
                        f"  Learning rates will now decay smoothly proportional to {decay_type}"
                    )

                if self.writer is not None:
                    self.writer.add_scalar(
                        "LearningRate/decay_phase_start", 1.0, self.training_step_count
                    )
                    self.writer.add_text(
                        "LearningRate/transitions",
    ...
    def train(self, num_training_steps: Optional[int] = None) -> List[Dict[str, float]]:
        ...
        if self.verbose:
            active = self.config.get_active_networks(self.training_step_count)
            print(
                f"[Warmup] Starting in stage {learner_state.prev_stage}: {learner_state.prev_stage_name} (active networks: {active})"
            )
        ...
        try:
            while self.training_step_count < num_training_steps:
                ...
                while (
                    ...
                    and self.training_step_count < num_training_steps
                ):
                    training_step_accumulator -= 1.0
                    ...
                    if (
                        self.config.memory_check_interval > 0
                        and self.training_step_count % self.config.memory_check_interval
                        == 0
                    ):
                    ...
                    if (
                        ...
                        and self.training_step_count - last_checkpoint_step
    ...
    def _get_policy_state_dict(self) -> Dict[str, Any]:
        ...
        if self.config.use_human_action_rnd and self.networks.human_rnd is not None:
            state_dict["human_rnd_predictor"] = (
                self.networks.human_rnd.predictor.state_dict()
            )
            state_dict["human_rnd_running_mean"] = (
                self.networks.human_rnd.running_mean.clone()
            )
            state_dict["human_rnd_running_var"] = (
                self.networks.human_rnd.running_var.clone()
            )
            state_dict["human_rnd_update_count"] = (
                self.networks.human_rnd.update_count.clone()
            )
        return state_dict
    ...
    def _load_policy_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        if "human_rnd_predictor" in state_dict and self.networks.human_rnd is not None:
            self.networks.human_rnd.predictor.load_state_dict(
                state_dict["human_rnd_predictor"]
            )
            self.networks.human_rnd.running_mean.copy_(
                state_dict["human_rnd_running_mean"]
            )
            self.networks.human_rnd.running_var.copy_(
                state_dict["human_rnd_running_var"]
            )
            self.networks.human_rnd.update_count.copy_(
                state_dict["human_rnd_update_count"]
            )
        ...
        while not stop_event.is_set():
            ...
            if self.config.max_env_steps_per_training_step is not None:
                while not stop_event.is_set():
                    ...
                    max_env = (
                        training_steps * self.config.max_env_steps_per_training_step
                    )
            ...
            env_steps_per_training_step = (
                self.config.max_env_steps_per_training_step or 1.0
            )
            ...
            if self._rnd_enabled() and self.config.rnd_sync_freq > 0:
                policy_sync_training_steps = min(
                    policy_sync_training_steps, self.config.rnd_sync_freq
                )

            # Convert to env steps: N training steps * M env_steps/training_step
            sync_freq_env_steps = max(
                1, int(policy_sync_training_steps * env_steps_per_training_step)
            )
            ...
            if steps_since_sync >= sync_freq_env_steps or local_policy_version < 0:
                with policy_lock:
                    ...
                    if current_version > local_policy_version:
                        policy_state = self._deserialize_policy_state(
                            shared_policy["state_dict"]
                        )
    ...
    def _learner_loop(
        ...
        if self.verbose:
            active = self.config.get_active_networks(self.training_step_count)
            print(
                f"[Learner] Starting in warmup stage {learner_state.prev_stage}: {learner_state.prev_stage_name} (active: {active})"
            )
            if self.writer is not None:
                import os
        ...
        if self.verbose:
            print(
                f"[Learner] Waiting for {self.config.async_min_buffer_size} transitions..."
            )
        ...
        if self.verbose:
            print(
                f"[Learner] Buffer ready with {len(self.replay_buffer)} transitions. Starting training."
            )
        ...
        if self.verbose:
            print(
                f"[Learner] Completed {self.training_step_count} training steps, {policy_updates} policy updates"
            )
    ...
    def _consume_transitions(
        self, transition_queue: mp.Queue, max_items: int = 100
    ) -> int:
        """
        ...
        while consumed < max_items:
            try:
                ...
                transition = Phase2Transition(
                    ...
                    transition_probs_by_action=trans_dict.get(
                        "transition_probs_by_action"
                    ),
    ...
    def load_all_networks(self, path: str, strict: bool = True) -> None:
        ...
        if "v_r_target" in checkpoint and self.networks.v_r_target is not None:
            self.networks.v_r_target.load_state_dict(
                checkpoint["v_r_target"], strict=strict
            )
        if "v_h_e_target" in checkpoint and self.networks.v_h_e_target is not None:
            self.networks.v_h_e_target.load_state_dict(
                checkpoint["v_h_e_target"], strict=strict
            )
        if "x_h_target" in checkpoint and self.networks.x_h_target is not None:
            self.networks.x_h_target.load_state_dict(
                checkpoint["x_h_target"], strict=strict
            )
        if "u_r_target" in checkpoint and self.networks.u_r_target is not None:
            self.networks.u_r_target.load_state_dict(
                checkpoint["u_r_target"], strict=strict
            )
        if "q_r_target" in checkpoint and self.networks.q_r_target is not None:
            self.networks.q_r_target.load_state_dict(
                checkpoint["q_r_target"], strict=strict
            )
    ...
    def get_v_h_e(
        self, state: Any, world_model: Any, human_agent_idx: int, goal: Any
    ) -> float:
        """
            ...
            with torch.no_grad():
                if isinstance(self.goal_sampler, TabularGoalSampler):
                    ...
                    probs_tensor = torch.tensor(
                        probs, device=self.device, dtype=v_h_e_values.dtype
                    )
                    weights_tensor = torch.tensor(
                        weights, device=self.device, dtype=v_h_e_values.dtype
                    )
                    x_h = (
                        probs_tensor * weights_tensor * (v_h_e_values**self.config.zeta)
                    ).sum()
                else:
                    ...
                    v_h_e_values = self.networks.v_h_e.forward_batch(
                        [state] * n_goal_samples,
                        goals,
                        [human_agent_idx] * n_goal_samples,
                        self.env,
                        self.device,
                    ).squeeze()
                    v_h_e_values = self.networks.v_h_e.apply_hard_clamp(v_h_e_values)
    ...
    def get_pi_r(
        self, state: Any, world_model: Any, beta_r: Optional[float] = None
    ) -> np.ndarray:
        """
    ...
    def get_u_r(self, state: Any, world_model: Any) -> float:
        """
