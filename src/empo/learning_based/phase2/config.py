...
class Phase2Config:
    ...
    rnd_bonus_coef_h: float = (
        0.1  # Human curiosity bonus coefficient (for human action RND)
    )
    ...
    use_human_action_rnd: bool = (
        False  # Enable human action RND (requires use_rnd=True)
    )
    ...
    warmup_x_h_steps: int = (
        1e4  # Duration of V_h^e + X_h stage (0 if x_h_use_network=False)
    )
    warmup_u_r_steps: int = (
        5e3  # Duration of V_h^e + X_h + U_r stage (0 if u_r_use_network=False)
    )
    ...
    env_steps_per_training_step: Optional[float] = (
        None  # Alternative way to specify ratio
    )
    ...
    lookup_default_v_h_e: float = (
        0.0  # V_h^e ∈ [0, 1] (probability) - 0 = pessimistic default
    )
    ...
    rnd_adaptive_lr_min: float = (
        0.1  # Minimum LR multiplier (prevents vanishing updates)
    )
    rnd_adaptive_lr_max: float = (
        10.0  # Maximum LR multiplier (prevents exploding updates)
    )
    ...
    def __post_init__(self):
        ...
        if (
            ...
            and self.use_rnd
            and not self.use_count_based_curiosity
        ):
            warnings.warn(
        ...
        if self.env_steps_per_training_step is not None:
            ...
            if self.training_steps_per_env_step == 1.0:
                self.training_steps_per_env_step = (
                    1.0 / self.env_steps_per_training_step
                )
    ...
    agent_position_feature_dim: int = (
        32  # Agent identity encoder: position encoding output dimension
    )
    agent_feature_dim: int = (
        32  # Agent identity encoder: agent feature encoding output dimension
    )
    ...
    def get_epsilon_r(self, step: int) -> float:
        ...
        # Linear decay
        decay_rate = (
            self.epsilon_r_start - self.epsilon_r_end
        ) / self.epsilon_r_decay_steps
    ...
    def get_epsilon_h(self, step: int) -> float:
        ...
        # Linear decay
        decay_rate = (
            self.epsilon_h_start - self.epsilon_h_end
        ) / self.epsilon_h_decay_steps
    ...
    def uses_trajectory_targets(self) -> bool:
        ...
        return (
            self.uses_trajectory_v_h_e_targets() or self.uses_trajectory_q_r_targets()
        )
    ...
    def get_effective_grad_clip(
        self, network_name: str, current_lr: float
    ) -> Optional[float]:
        """
    ...
    def is_in_rampup(self, step: int) -> bool:
        ...
        return (
            self._warmup_v_r_end
            <= step
            < self._warmup_v_r_end + self.beta_r_rampup_steps
        )
    ...
    def get_stage_transition_steps(self) -> list:
        ...
        if self._warmup_v_h_e_end > 0:
            if self.x_h_use_network:
                ...
                transitions.append(
                    (self._warmup_v_h_e_end, "Q_r starts (X_h computed from V_h^e)")
                )
        ...
        if self.beta_r_rampup_steps > 0:
            transitions.append(
                (self._warmup_v_r_end + self.beta_r_rampup_steps, "β_r ramp complete")
            )
        return transitions
    ...
    def format_stages_table(self) -> str:
        ...
        lines.append("-" * 70)
        lines.append(
            f"{'Stage':<6} {'Name':<25} {'Duration':>12} {'End Step':>12} {'Networks'}"
        )
        lines.append("-" * 70)
    ...
    def should_use_lookup_table(self, network_name: str) -> bool:
        ...
        lookup_flags = {
            ...
            "v_r": self.use_lookup_v_r
            and self.v_r_use_network,  # Only if V_r network is used
            "v_h_e": self.use_lookup_v_h_e,
            "x_h": self.use_lookup_x_h
            and self.x_h_use_network,  # Only if X_h network is used
            "u_r": self.use_lookup_u_r
            and self.u_r_use_network,  # Only if U_r network is used
        }
