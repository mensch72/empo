    ...
    for state in pol1:
        ...
        for agent_idx in pol1[state]:
            if agent_idx not in pol2[state]:
                differences.append(
                    f"Agent {agent_idx} missing in parallel for state {state}"
                )
                continue
            ...
            for goal in pol1[state][agent_idx]:
                if goal not in pol2[state][agent_idx]:
                    differences.append(
                        f"Goal {goal} missing in parallel for state {state}, agent {agent_idx}"
                    )
                    continue
...
def test_parallel_correctness_small():
    ...
    result_seq = compute_human_policy_prior(
        wm, human_agent_indices, goal_gen, parallel=False
    )
    ...
    result_par = compute_human_policy_prior(
        wm, human_agent_indices, goal_gen, parallel=True, level_fct=lambda s: s[0]
    )
    ...
    assert not differences, (
        f"Found {len(differences)} differences between sequential and parallel:\n"
        + "\n".join(differences[:10])
    )
...
def test_v_values_visibility():
    ...
    states, state_to_idx, successors, transitions = wm.get_dag(
    ...
    for state_idx, state in enumerate(states):
        ...
        if transitions[state_idx]:  # Non-terminal
            ...
            if level in succ_levels or any(l <= level for l in succ_levels):
                print(
                    f"WARNING: State at level {level} has successors at levels {succ_levels}"
                )
