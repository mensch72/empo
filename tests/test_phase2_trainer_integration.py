        ...
        *,
        ...
        class MockTrainer:
            ...
            def check_goal_achieved(self, next_state, human_idx, goal):
                captured.setdefault("goal_checks", []).append(
                    (next_state, human_idx, goal)
                )
                return achieved
...
class TestTensorBoardLogging:
    ...
    def test_q_r_mode_specific_loss_stats_reach_tensorboard(
        self, stats, expected_tag, unexpected_tag
    ):
        ...
        class MockTrainer:
            ...
            def __init__(self):
                ...
                self.profiler = SimpleNamespace(
                    section=lambda _name: nullcontext(), step=lambda: None
                )
        ...
        trainer = MockTrainer()
        learner_state = BasePhase2Trainer._LearnerState(
            prev_stage=4, prev_stage_name="q_r"
        )
...
class TestModelBasedTargets:
    ...
    def test_next_state_none_when_model_based(self):
        ...
        config = Phase2Config(
            use_model_based_targets=True, batch_size=4, buffer_size=100
        )
        ...
        transition_probs = (
            {0: [(1.0, "successor_state")]} if config.use_model_based_targets else None
        )
    ...
    def test_next_state_stored_for_trajectory_targets(self):
        ...
        stored_next_state = (
            "some_state" if config.should_store_sampled_next_state() else None
        )
        transition = Phase2Transition(
            state="current_state",
            robot_action=(0,),
    ...
    def test_next_state_stored_when_not_model_based(self):
        ...
        config = Phase2Config(
            use_model_based_targets=False, batch_size=4, buffer_size=100
        )

        stored_next_state = "some_state" if not config.use_model_based_targets else None
        transition_probs = (
            None if not config.use_model_based_targets else {0: [(1.0, "successor")]}
        )
    ...
    def test_no_successor_states_stored_in_transitions(self):
        ...
        for _ in range(10):
            ...
            stored_next_state = (
                None if config.use_model_based_targets else "actual_next_state"
            )
            ...
            transition_probs = (
                ...
                if config.use_model_based_targets
                else None
            )
            ...
            assert (
                transition.transition_probs_by_action is not None
            ...
            assert (
                len(all_successors) > 0 or 2 in transition.transition_probs_by_action
...
class TestTrajectoryTargets:
    ...
    def _build_v_h_e_trainer(
        self, mode, n_step, achieved_states=None, bootstrap_values=None, gamma_h=0.5
    ):
        ...
        trainer.check_goal_achieved = (
            lambda state, human_idx, goal: state in achieved_states
        )
        return trainer

    def _build_q_r_trainer(
        self, mode, n_step, u_r_values, v_r_values=None, gamma_r=0.5
    ):
        trainer = self._TrajectoryTrainer()
        trainer.device = "cpu"
        trainer.env = None
        ...
        for idx, (state, next_state) in enumerate(
            ...
            trainer.replay_buffer.push(
                **self._make_transition(
                    state, next_state, env_step_index=idx, terminal=terminal
                ).__dict__
            )
    ...
    def test_q_r_episode_drops_bootstrap_at_terminal(self):
        ...
        trainer.replay_buffer.push(
            **self._make_transition(
                "s1", "s2", env_step_index=1, terminal=True
            ).__dict__
        )
    ...
    def test_q_r_prediction_stats_use_mode_specific_loss_key(
        self, mode, present_key, absent_key
    ):
    ...
    def test_q_r_search_policy_distillation_can_relabel_replay_with_fresh_search(self):
        """Replay-state relabeling should refresh missing MCTS targets before distillation."""
        trainer = self._build_q_r_distillation_trainer(coef=0.5)
        trainer.config = Phase2Config(
            use_model_based_targets=False,
            q_r_target_mode="n_step",
        ...
        assert relabel_calls == [
            (
                ...
                {0: "goal"},
                2.0,
                {"add_root_noise": False, "sample_action": False},
            )
        ]
...
class TestMCTSPolicyImprovement:
    ...
    class _StaticQNetwork:
        ...
        def forward(self, state, env, device):
            return torch.tensor(
                [self.q_values_by_state[state]], dtype=torch.float32, device=device
            )
    ...
    class _StaticVrTarget:
        ...
        def forward(self, state, env, device):
            return torch.tensor(
                [self.values[state]], dtype=torch.float32, device=device
            )
    ...
    class _SearchTrainer:
        ...
        _sample_robot_action_with_stats = (
            BasePhase2Trainer._sample_robot_action_with_stats
        )
        _run_mcts_policy_search = BasePhase2Trainer._run_mcts_policy_search
        _sample_human_actions_for_search = (
    ...
    def test_collect_transition_stores_mcts_root_statistics(self):
        ...
        class MockTrainer:
            ...
            _precompute_transition_probs = (
                BasePhase2Trainer._precompute_transition_probs
            )
            _build_joint_action_profile = BasePhase2Trainer._build_joint_action_profile
        ...
        trainer = MockTrainer()
        trainer._sample_robot_action_with_stats = (
            lambda state, goals, transition_probs_by_action: (
                (1,),
                SimpleNamespace(
                    policy=(0.1, 0.9),
                    root_value=-0.4,
...
class TestAsyncRNDSync:
    ...
    def test_rnd_networks_included_in_policy_state_dict(self):
        ...
        class MockTrainer:
            ...
            def _get_policy_state_dict(self):
                ...
                if self.config.use_rnd and self.networks.rnd is not None:
                    state_dict["rnd_predictor"] = (
                        self.networks.rnd.predictor.state_dict()
                    )
                    state_dict["rnd_running_mean"] = (
                        self.networks.rnd.running_mean.clone()
                    )
                    state_dict["rnd_running_var"] = (
                        self.networks.rnd.running_var.clone()
                    )
                    state_dict["rnd_update_count"] = (
                        self.networks.rnd.update_count.clone()
                    )
                if (
                    self.config.use_human_action_rnd
        ...
        assert (
            "rnd_predictor" in state_dict
        ), "RND predictor should be in state dict when use_rnd=True"
        assert (
            "rnd_running_mean" in state_dict
        ), "RND running_mean should be in state dict"
        assert (
            "rnd_running_var" in state_dict
        ), "RND running_var should be in state dict"
        assert (
            "rnd_update_count" in state_dict
        ), "RND update_count should be in state dict"

        # Verify human RND is included
        assert (
            "human_rnd_predictor" in state_dict
        ), "Human RND predictor should be in state dict when use_human_action_rnd=True"
        assert (
            "human_rnd_running_mean" in state_dict
        ), "Human RND running_mean should be in state dict"
        assert (
            "human_rnd_running_var" in state_dict
        ), "Human RND running_var should be in state dict"
        assert (
            "human_rnd_update_count" in state_dict
        ), "Human RND update_count should be in state dict"
    ...
    def test_rnd_networks_can_be_loaded_from_state_dict(self):
        ...
        assert (
            target_rnd.update_count.item() != 1000
        ), "Target should start with different count"
        assert (
            target_human_rnd.update_count.item() != 2000
        ), "Target should start with different count"
        ...
        assert (
            target_rnd.update_count.item() == 1000
        ), "RND update_count should be restored"
        assert torch.allclose(
            target_rnd.running_mean, torch.full_like(target_rnd.running_mean, 0.5)
        ), "RND running_mean should be restored"
        assert torch.allclose(
            target_rnd.running_var, torch.full_like(target_rnd.running_var, 2.0)
        ), "RND running_var should be restored"

        assert (
            target_human_rnd.update_count.item() == 2000
        ), "Human RND update_count should be restored"
        assert torch.allclose(
            target_human_rnd.running_mean,
            torch.full_like(target_human_rnd.running_mean, 0.7),
        ), "Human RND running_mean should be restored"
        assert torch.allclose(
            target_human_rnd.running_var,
            torch.full_like(target_human_rnd.running_var, 3.0),
        ), "Human RND running_var should be restored"

        # Verify predictor weights were restored
        for target_param, source_param in zip(
            target_rnd.predictor.parameters(), source_rnd.predictor.parameters()
        ):
            assert torch.allclose(
                target_param, source_param
            ), "RND predictor weights should match"

        for target_param, source_param in zip(
            target_human_rnd.predictor.parameters(),
            source_human_rnd.predictor.parameters(),
        ):
            assert torch.allclose(
                target_param, source_param
            ), "Human RND predictor weights should match"
    ...
    def test_rnd_sync_freq_config(self):
        ...
        assert (
            config.rnd_sync_freq <= config.actor_sync_freq
