    ...
    for state in pol1:
        ...
        for agent_idx in pol1[state]:
            if agent_idx not in pol2[state]:
                differences.append(f"Agent {agent_idx} missing in pol2 for state {state}")
                continue
            
            for goal in pol1[state][agent_idx]:
                if goal not in pol2[state][agent_idx]:
                    differences.append(f"Goal {goal} missing in pol2 for state {state}, agent {agent_idx}")
                    continue
...
def test_custom_believed_others_policy_parallel():
    ...
    assert len(differences) == 0, f"Found {len(differences)} differences: {differences[:5]}"
