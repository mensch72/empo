...
def test_phase2_config():
    ...
    assert (
        config.get_epsilon_r(config.epsilon_r_decay_steps // 2) < config.epsilon_r_start
    )
    ...
    assert (
        config.get_epsilon_h(config.epsilon_h_decay_steps // 2) < config.epsilon_h_start
    )
...
def test_phase2_replay_buffer():
    ...
    for i in range(20):
        buffer2.push(
            state, (i % 4, i % 4), goals, goal_weights, human_actions, next_state
        )
...
def test_multigrid_human_goal_achievement_network():
    ...
    target3 = v_h_e_network.compute_td_target(
        goal_not_achieved, next_v, terminal=terminal_flag
    )
    assert (
        target3.item() == 0.0
    ), "When terminal and goal not achieved, target should be 0"
    print(f"  ✓ TD target when terminal: {target3.item()}")

    # Case 4: Goal achieved AND terminal (terminal flag should be ignored)
    target4 = v_h_e_network.compute_td_target(
        goal_achieved, next_v, terminal=terminal_flag
    )
    assert (
        target4.item() == 1.0
    ), "When goal achieved, target should be 1 regardless of terminal"
    ...
    target5 = v_h_e_network.compute_td_target(
        goal_batch, next_v_batch, terminal=terminal_batch
    )
...
def test_multigrid_aggregate_goal_ability_network():
    ...
    # Test forward pass
    x_h = x_h_network.forward(state, world_model, human_agent_idx=0, device="cpu")
...
def test_multigrid_intrinsic_reward_network():
    ...
    # Test forward pass
    y, u_r = u_r_network.forward(state, world_model, device="cpu")
...
def test_multigrid_robot_value_network():
    ...
    # Test forward pass
    v_r = v_r_network.forward(state, world_model, device="cpu")
    ...
    assert (
        negative_values < 0
    ).all(), "ensure_negative should make all values negative"
    print("  ✓ ensure_negative: all values negative")
...
def test_power_law_policy():
    ...
    assert (
        policy_high[0, 0] > policy_low[0, 0]
    ), "Higher beta should give more weight to best action"
    print(
        f"  ✓ Higher beta concentrates probability: {policy_low[0, 0]:.4f} < {policy_high[0, 0]:.4f}"
    )
...
def test_soft_clamp_in_phase2():
    ...
    assert torch.allclose(
        test_values.grad, torch.ones(3), atol=0.01
...
def test_ensure_negative():
    ...
    print("  ✓ Q_r ensure_negative: all values negative")
    print("  ✓ V_r ensure_negative: all values negative")
