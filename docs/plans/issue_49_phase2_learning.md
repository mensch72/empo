...
class NeuralIntrinsicRobotReward(nn.Module):
    ...
    The intermediate y = Σ_h X_h(s)^{-ξ} is in [|H|, ∞) since:
    - X_h > 0 (aggregate of positive probabilities)
    - X_h^{-ξ} > 0 for any ξ > 0
    ...
    def forward(self, state_encoding) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        log_y = self.head(state_encoding)
        y = torch.exp(log_y)  # y > 0
        y = y + 1.0  # Ensure y >= 1
...
2. At each step:
   ...
   - Sample human actions `a_H ~ π_H(s, g)` from Phase 1 policy
...
Target networks are updated every `target_update_freq` steps.
...
class Phase2Config:
    ...
    xi: float = 1.0     # ξ - inter-human inequality aversion (>0)
    eta: float = 1.1    # η - intertemporal inequality aversion (>1)
    
    # Robot policy
    beta_r: float = 1.0  # Robot rationality (power-law exponent)
