...
| [`her_goal_relabeling.md`](./her_goal_relabeling.md) | Proposed | — | Design for hindsight goal relabeling of `V_h^e` with per-decision importance sampling, self-normalization, and shortest-path goal selection. No implementation has landed yet. |
...
| [`phase2_multistep_targets_and_mcts.md`](./phase2_multistep_targets_and_mcts.md) | In progress | Landed in code: `src/empo/learning_based/phase2/config.py`, `replay_buffer.py`, `trainer.py`; tests in `tests/test_phase2.py`, `tests/test_phase2_trainer_integration.py` | Optional trajectory targets, acting-time MCTS, root search noise, delayed MCTS gating, replay-state search relabeling, and async replay freshness controls are present, while evaluation and broader search comparisons remain open. |
