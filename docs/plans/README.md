...
| [`phase2_multistep_targets_and_mcts.md`](./phase2_multistep_targets_and_mcts.md) | In progress | Landed in code: `src/empo/learning_based/phase2/config.py`, `replay_buffer.py`, `trainer.py`; tests in `tests/test_phase2.py`, `tests/test_phase2_trainer_integration.py` | Optional trajectory targets, acting-time MCTS, root search noise, delayed MCTS gating, and async replay freshness controls are present, while search-aware training refinements and evaluation remain open. |
